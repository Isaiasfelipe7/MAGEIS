...
class Ativo:
    ...
    def cadastrar_ativos(self):
        ...
        cur = conec.cursor()
        cur.execute('INSERT INTO ativos (id, nome, qtd, valor) VALUES (%s,%s, %s, %s)', (self.__id,self.nome, self.qtd, self.valor))
        conec.commit()
    ...
    def exluir_ativo(self):
        ...
        conec = GerenciarBanco.conectar_database(self)
        cur = conec.cursor()
        cur.execute('SELECT * FROM ativos WHERE id = %s', (id,))
        ativo = cur.fetchone()
        if ativo is None:
    ...
    def listar_transacoes(self):
        ...
        if not transacao:
            ...
            for trans in transacao:
                cur.execute('SELECT nome FROM ativos WHERE id=%s', (trans[6],))
